...
| A9 | Lambda Docker base images (not digest-pinned) | `lambdas/Dockerfile` line 3 (`FROM python:3.12`), `lambdas/taxon-indexing-concurrency-manager/Dockerfile` line 1 (`FROM node:20`), `local-base-images/Dockerfile.python-base` line 1 (`lambda/python:3.12`), `local-base-images/Dockerfile.node-base` line 1 (`lambda/nodejs:20`) | Bare floating major tags; `pinDigests:true` *would* pin them once Renovate runs, but major tags won't auto-advance | Bump base tags by hand until Renovate is confirmed live (then verify it pinned digests). Note: the `lambda/*` bases are Amazon Linux 2023 -> use `dnf`, not `yum` |
...
| A12 | Python version pin | `.python-version` → `3.12` (consumed by `check.yml` `setup-python` via `python-version-file`) | No Renovate manager is configured for `.python-version` (the only custom manager targets `.terraform-version`) | Hand-edit; lambda runtimes + base images are now aligned at py3.12 / node20 |
