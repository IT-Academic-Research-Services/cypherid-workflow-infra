...
# AMI churn control (SMP-1745): the data source above resolves the AWS-published "latest"
# ECS-optimized AMI, which changes every time AWS publishes a new image and would force-replace
# every Batch compute environment on the next plan (image_id forces replacement). Rather than
# pin the id in config and advance it on a schedule, the launch template and CEs below read the
# data source directly and declare `image_id` in their lifecycle ignore_changes: the AMI is
# chosen at create/replace time and never diffs afterward, so publishes never churn the plan.
...
resource "aws_launch_template" "index_generation" {
  ...
  # Freeze the AMI: chosen at create/replace time, never diffs on an AWS AMI publish. See the
  # "AMI churn control" note above. Roll a newer AMI with `terraform apply -replace`.
  lifecycle {
...
resource "aws_batch_compute_environment" "index_generation" {
  ...
  lifecycle {
    ...
    ignore_changes = [
      ...
      # Freeze the AMI: chosen at create/replace time, never diffs on an AWS AMI publish.
      # See the "AMI churn control" note above. Roll deliberately with `apply -replace`.
